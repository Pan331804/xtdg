    ...
    for ogloszenie in ogloszenia:
        ...
        if data_div and 'dzisiaj' in data_div.text.lower():
            try:
                ...
                ogloszenie_datetime = teraz.replace(hour=godzina_obj.hour, minute=godzina_obj.minute, second=0, microsecond=0)
                
                roznica = teraz - ogloszenie_datetime
                
                # Sprawdź, czy ogłoszenie jest nie starsze niż 15 minut
                if timedelta(minutes=0) <= roznica <= timedelta(minutes=15):
